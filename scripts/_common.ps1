...
function Repair-SkillSyncState {
    <#
      .SYNOPSIS 自愈式修复 lastSyncTree 状态失效（多机场景）
      .DESCRIPTION
        单用户多机场景下，机器 A sync 后写入 lastSyncTree，机器 B pull 下来后
        registry 里的 lastSyncTree 对不上机器 B 的工作区。此函数检测并自动修复：
    ...
    $vendored = @($registry.skills | Where-Object { $_.origin -eq 'vendored' })
    if ($vendored.Count -eq 0) { return }

    # 检查工作区是否干净（只看 skills/ 下）
    $skillsStatus = Invoke-Git @('status', '--porcelain', 'skills/') -AllowFail
    if ($skillsStatus) { return }  # 有未提交改动，不自动修复
    ...
    foreach ($e in $vendored) {
        ...
        $currentTree = Get-SkillTreeHash $e.name
        if (-not $currentTree) { continue }

        if ($e.lastSyncTree -and $e.lastSyncTree -ne $currentTree) {
            # tree hash 对不上，但工作区干净 → 是其他机器 sync 后的状态
            $e.lastSyncTree = $currentTree
            $e.lastSync = Get-UtcNow
            $registry = Set-SkillEntry -Registry $registry -Entry $e
            $fixed += $e.name
        }
    }
    ...
    if ($fixed.Count -gt 0) {
        ...
        if (-not $Silent) {
            Write-Ok "自动修复 $($fixed.Count) 个 skill 的同步状态（多机环境）"
            $fixed | ForEach-Object { Write-Hint $_ }
        }
    }
}

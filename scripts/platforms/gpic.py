...
CONFIG = {
    "name": "粤港澳知识产权大数据平台",
    "base_url": "https://search.gpic.gd.cn",
}
...
class GpicBrowser:
    ...
    def search(self, patent_number: str):
        """搜索专利"""
        self.page.goto(f"{self.BASE_URL}/search-gpic/pldb/common/search")
        self.page.wait_for_load_state("networkidle")
        
        self.page.fill('input[type="text"]', patent_number)
        self.page.click('button:has-text("查询"), button:has-text("搜索")')
    ...
    def download_pdf(self, patent_number: str, output_dir: str = ".") -> str:
        ...
        # 尝试点击详情
        try:
            self.page.click('.result-item a, a[href*="detail"]')
            self.page.wait_for_load_state("networkidle")
        except:
            pass

        print(f"⚠️ 粤港澳平台 PDF 下载尚未实现，未保存文件。推荐改用 Google Patents：")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="粤港澳知识产权平台专利下载")
    parser.add_argument("patents", nargs="+", help="专利号列表")
    _creds = load_creds("gpic")
    ...
    downloader = GpicBrowser(args.headless)

...
7. **尾部作者签名区（作者自填，仅末尾一处）**：**默认不写死任何人名**，用占位署名让用户替换成自己的。
   ...
   - **文末公众号二维码（本库自用，固定附加，勿提交给上游）**：签名区之后必须再加一块二维码。每次都加，不要向用户要图，也不要把图片复制到文章目录。源文件是 `<SKILL_ROOT>/assets/account-qr.jpg`（脑后插管公众号二维码）。运行 `python <SKILL_ROOT>/scripts/account_qr_block.py`，把它打印的整个 `<section>` 原样接在签名区后面。不要手写、截断或改写 src 里的 data URI。卡片里的「长按识别二维码关注」保持原句。这是真实图片，不适用下面「占位图残留就删掉」的规则。
...
- **占位图残留**：签名区/CTA 组件里若带名片图占位（如 `<img src="...名片或引导图URL">`），没有真实图片 URL 时整行删掉，不要把占位符留在产物里。上面的自用公众号二维码不在此列，按脚本输出保留。
